...
class CaseStatusIT extends CustomAbstractAppTest {
	...
	void test4_GetStatusByOrgNr() throws JsonProcessingException, ClassNotFoundException {
		...
		assertTrue(getStatusResponse.stream().anyMatch(caseStatus -> "Begäran om anstånd".equals(caseStatus.getStatus())));
		assertTrue(getStatusResponse.stream().anyMatch(caseStatus -> "ANSÖKAN".equals(caseStatus.getStatus())));
	}
}
