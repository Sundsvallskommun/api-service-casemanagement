...
public class EDPFutureService {
	...
	/**
	 * Extracts the order type with the type "SA_TRÄDGSÄCK" from the GetRenhOrderTypesForServiceV17Response. It looks for an
	 * order type that matches the given serviceId and has the type "SA_TRÄDGSÄCK", and returns it. If no such order type is
	 * found, it throws
	 * an exception.
	 *
	 * @param  response GetRenhOrderTypesForServiceV17Response containing the list of order types associated with the
	 *                  serviceId.
	 * @return          the order type with the type "SA_TRÄDGSÄCK" for the given serviceId.
	 */
	private OrderTypeV14 getOrderType(final GetRenhOrderTypesForServiceV17Response response, final String quantity) {
		var orderType = response.getGetRenhOrderTypesForServiceV17Result().getResultValue().getOrderTypeV17().stream()
			.filter(service -> "SA_TRÄDGSÄCK".equals(service.getType()))
			.findFirst()
			.orElseThrow(() -> Problem.valueOf(BAD_GATEWAY, "Failed to retrieve order type from EDP Future. No order type found with the given type."));
	 ...
	 * @param orderType  the order type associated with the serviceId and with the type "SA_TRÄDGSÄCK".
	 */
