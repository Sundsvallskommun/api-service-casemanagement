...
public class RiskClassService {
    ...
    private String extractOrgNr(EnvironmentalCaseDTO eCase) {
        return CaseUtil.getSokigoFormattedOrganizationNumber(eCase.getStakeholders().stream().map(stakeholderDTO -> {
    ...
    private String searchFacility(String orgNr, String facilityCollectionName) {
        ...
        var facilityTypeFilter = new FacilityFacilityTypeIdsFilterSvcDto().withFacilityTypeIds("4958BC00-76E8-4D5B-A862-AAF8E815202A");
        
        var facilityStatusFilter = new FacilityFacilityStatusIdsFilterSvcDto().withFacilityStatusIds(new ArrayOfguid().withGuid("88E11CAA-DF35-4C5E-94A8-3C7B0369D8F2", //Anmäld/Ansökt
        ...
        var notFacilityStatusFilters = new FacilityNotFilterSvcDto().withFilter(new FacilityFacilityStatusIdsFilterSvcDto().withFacilityStatusIds(new ArrayOfguid().withGuid(List.of("9A748E4E-BD7E-481A-B449-73CBD0992213", //Upphörd/Skrotad
            "80FFA45C-B3DF-4A10-8DB3-A042F36C64B7")))); // Makulerad
        
        
        var orgFilter = new FacilityPartyOrganizationNumberFilterSvcDto().withOrganizationNumber(orgNr);
        
        var result = Optional.ofNullable(minutMiljoClient.searchFacility(new SearchFacility().withSearchFacilitySvcDto(new SearchFacilitySvcDto().withFacilityFilters(new ArrayOfFacilityFilterSvcDto().withFacilityFilterSvcDto(facilityStatusFilter, facilityTypeFilter, notFacilityStatusFilters, orgFilter)))).getSearchFacilityResult().getSearchFacilityResultSvcDto()).orElseThrow(() -> Problem.valueOf(Status.NOT_FOUND, "Could not find facility "));
        
        return result.stream().filter(resultSvcDto -> resultSvcDto.getFacilityCollectionName().equals(facilityCollectionName)).findFirst().orElseThrow(() -> Problem.valueOf(Status.BAD_REQUEST, ("Could not match facilityCollectionName: %s to a facility belonging to organization with organizationNumber: %s").formatted(facilityCollectionName, orgNr))).getFacilityId();
    }
    
    private void addFacilityToCase(String facilityId, String caseId) {
        minutMiljoClient.addFacilityToCase(new AddFacilityToCase().withFacilityId(facilityId).withCaseId(caseId));
        
    }
    
    private SaveFoodFacility2024RiskClassData createSaveRiskClassObject(String facilityId, String caseId, EnvironmentalCaseDTO dto) {
        
        return new SaveFoodFacility2024RiskClassData().withModel(new SaveRiskClass2024DataDto().withCaseId(caseId).withFacilityId(facilityId).withMainOrientationSlvCode(dto.getExtraParameters().get(MAIN_ORIENTATION_ID)).withProductionSizeSlvCode(dto.getExtraParameters().get(PROD_SIZE_ID)).withIsSeasonal(Optional.ofNullable(dto.getExtraParameters().get(IS_SEASONAL)).orElse("").equalsIgnoreCase("true")).withSeasonalNote(dto.getExtraParameters().get(SEASONAL_NOTE)).withActivities(mapActivities(dto)).withProductGroups(mapProductGroups(dto.getExtraParameters().get(PRODUCT_GROUPS))).withThirdPartyCertifications(mapThirdPartyCertifications(dto.getExtraParameters().get(THIRD_PARTY_CERTS))));
    }
    ...
    private ArrayOfSaveRiskClass2024ActivityDto mapActivities(EnvironmentalCaseDTO dto) {
        ...
        return new ArrayOfSaveRiskClass2024ActivityDto().withSaveRiskClass2024ActivityDto(activities.stream().map(ActivitySlv -> new SaveRiskClass2024ActivityDto().withSlvCode(ActivitySlv)).filter(activityDto -> !activityDto.getSlvCode().isEmpty()).toList());
    }
    ...
    private ArrayOfSaveRiskClass2024ProductGroupDto mapProductGroups(String productGroupIdString) {
        ...
        return new ArrayOfSaveRiskClass2024ProductGroupDto().withSaveRiskClass2024ProductGroupDto(productGroupIds.stream().map(productGroupId -> new SaveRiskClass2024ProductGroupDto().withSlvCode(productGroupId)).toList());
    }
    ...
    private ArrayOfSaveRiskClass2024CertificationDto mapThirdPartyCertifications(String thirdPartyCertString) {
        ...
        return new ArrayOfSaveRiskClass2024CertificationDto().withSaveRiskClass2024CertificationDto(dtos.stream().map(dto -> new SaveRiskClass2024CertificationDto().withThirdPartyCertificationText(dto)).toList());
    }
    
    private List<String> splitString(String string) {
        return Arrays.stream(string.split(" ")).map(s -> s.split(",")).flatMap(Arrays::stream).toList();
    }
